...
# MAGIC **Core evaluation logic - CLEANED VERSION**

# COMMAND ----------

# =============================================================================
# LLM JUDGE EVALUATOR CLASS - PRODUCTION READY WITH PROPER ESCAPING
# =============================================================================
...
class LLMJudgeEvaluator:
    """
    Main evaluator class with automatic LLM routing and proper prompt escaping.
    
    Features:
    - Proper handling of JSON examples in prompts (no more KeyError!)
    - Supports Databricks LLM endpoints (auto-discovery)
    - Supports OpenAI API endpoints
    ...
    def _init_openai_llm(self):
        ...
        try:
            ...
            # Try to use the client from Cell 3
    ...
    def _get_ground_truth_for_metric(self, metric: MetricConfig, sample_idx: int) -> str:
        ...
        try:
            ...
            print(f"   🔍 Looking for {metric.ground_truth_column} in {len(files)} files")
                ...
                if filename in self.ground_truth_data:
                    ...
                    if metric.ground_truth_column in df.columns:
                        if sample_idx < len(df):
                            ...
                            print(f"   ✅ Found ground truth in {filename}")
                    ...
                    print(f"   ⚠️ File {filename} not loaded")
    ...
    def _escape_prompt_template(self, template: str) -> str:
        ...
        This is THE KEY FIX for the format string issue!
        
        Strategy:
        1. Identify and temporarily replace actual placeholders ({prompt}, {response}, {ground_truth})
        2. Escape all remaining curly braces by doubling them (so JSON examples work)
        3. Restore the actual placeholders
        
        Example:
        Input:  "Check {prompt}. Return: {\"score\": 1}"
        Output: "Check {prompt}. Return: {{\"score\": 1}}"
        """
    ...
    def evaluate_single(self, prompt: str, response: str, ground_truth_data: dict, metric: MetricConfig, sample_idx: int = 0) -> dict:
        ...
        try:
            ...
            # CRITICAL FIX: Properly escape the prompt template before formatting
            safe_template = self._escape_prompt_template(metric.prompt_template)
            
            # Now format safely - JSON examples won't cause KeyError!
...
def display_evaluation_results(results_df: pd.DataFrame):
    ...
    for _, row in sample_results.iterrows():
        print(f"Sample: {row['sample_id']} | Metric: {row['metric_name']} | Score: {row['score']:.2f} | Status: {row['status']}")
        print(f"Explanation: {row['explanation'][:100]}...")
        print()
    
    print("="*80)

print("✅ LLM Judge Evaluator class defined (cleaned version with proper escaping)")
